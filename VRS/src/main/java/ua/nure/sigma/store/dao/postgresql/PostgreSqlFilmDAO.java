...
public class PostgreSqlFilmDAO implements FilmDAO {
    ...
    private static final String SQL_SELECT_FROM_FILM_BY_ID =
            "Select films.ID ,TITLE, YEAR, DESCRIPTION, COVER, AMOUNT, GENERAL_PRICE, RENT_PRICE, BONUS_FOR_RENT, coalesce(rented.rentedCopies, 0) as rentedCp from films LEFT OUTER JOIN (SELECT ID, sum(count) as rentedCopies FROM FILM_AT_RENT WHERE ACCEPTED_DATE IS NULL group by ID) rented on rented.ID = films.ID WHERE FILMS.ID = ? ";
    private static final String SQL_SELECT_FROM_FILMS_ALL_FILM =
            "Select films.ID ,TITLE, YEAR, DESCRIPTION, COVER, AMOUNT, GENERAL_PRICE, RENT_PRICE, BONUS_FOR_RENT, coalesce(rented.rentedCopies,  0) as rentedCp from films LEFT OUTER JOIN (SELECT ID, sum(count) as rentedCopies FROM FILM_AT_RENT WHERE ACCEPTED_DATE IS NULL group by ID) rented on rented.ID = films.ID";
