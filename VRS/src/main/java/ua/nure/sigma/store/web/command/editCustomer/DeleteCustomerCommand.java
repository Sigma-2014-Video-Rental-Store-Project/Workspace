...
public class DeleteCustomerCommand extends Command {
	private static final Logger LOG = Logger
			.getLogger(DeleteCustomerCommand.class);

	/**
	 * Deletes {@code Customer} object by the specified id from the current
	 * database. Processes {@code null} id and unparsable id exceptions.
	 */
	@Override
	public String execute(HttpServletRequest request,
			HttpServletResponse response) throws IOException, ServletException {
		LOG.debug("DeleteCustomerCommand started.");
		String customerIdString = request
				.getParameter(EditCustomerCommand.CUSTOMER_ID_PARAM_NAME);
		...
		try {
			...
			request.getSession().setAttribute("errorMessage",
					"Can not delete user with rented films");
			return Paths.COMMAND_EDIT_CUSTOMER + "&customerId=" + customerId + "&get=true";
		}
		if (LOG.isDebugEnabled()) {
			LOG.debug(String
					.format("Next customer with id equals to '%d' has been deleted from the database.",
							customerId));
